...
qna_data = datasets.load_from_disk('data/bert_bobby_dataset')
tokenizer = get_tokenizer(TRAINING_PARAMS['MODEL'])

def main():
  df_train = qna_data['train'].remove_columns(['name', 'sourceName', 'sourceUrl', 'faqLabels'])
  
  df_train = df_train.train_test_split(test_size=0.15)
  ...
  for epoch in range(TRAINING_PARAMS['EPOCHS']):
      ...
      if val_acc > best_accuracy:
          torch.save(model.state_dict(), 'models/qna_classifier_2023-10-05.bin')
